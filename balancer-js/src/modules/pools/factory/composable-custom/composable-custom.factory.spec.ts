...
import composableCustomFactoryAbi from '@/lib/abi/ComposableCustomFactory.json';
...
const factoryAddress = `${BALANCER_NETWORK_CONFIG[network].addresses.contracts.composableCustomPoolFactory}`;
...
describe('creating composable stable pool', async () => {
  ...
  const composableCustomPoolFactory = balancer.pools.poolFactory.of(
    PoolType.ComposableCustom
  );
  context('create', async () => {
    ...
    it('should create a pool', async () => {
      const { to, data } = composableCustomPoolFactory.create({
        factoryAddress,
        name,
        symbol,
        tokenAddresses,
      ...
      const composableCustomFactoryInterface = new Interface(
        composableCustomFactoryAbi
      );
      ...
      const poolCreationEvent: LogDescription | null | undefined = receipt.logs
        ...
        .map((log) => {
          try {
            return composableCustomFactoryInterface.parseLog(log);
          } catch (error) {
            console.error(error);
            return null;
          }
